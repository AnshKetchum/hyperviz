...
class UniversalTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_hidden_states: bool = False):
        ...
        Args:
            x: (B, T)  token indices
            return_hidden_states: if True, include per-step hidden states in aux
        ...
        Returns:
            ...
            aux: dict with keys
                ...
                'hidden_states' — list of (B, T, E), one per step (only if return_hidden_states)
        """
        B, T = x.shape
        device = x.device
        ...
        for step in range(self.max_steps):
            ...
            if return_hidden_states:
                hidden_states.append(o.detach().cpu())
...
if __name__ == "__main__":
    ...
    # Viz pass
    print("Collecting 1000 trajectories...")
    viz = Visualizer(save_directory="viz")
    dataset_viz = DummyDataset(vocab_size, T, 1000)
    dataloader_viz = DataLoader(dataset_viz, batch_size=1, shuffle=False)
    model.eval()
    with torch.no_grad():
        for i, (x, _) in enumerate(dataloader_viz):
            if i >= 1000:
                break
            _, aux = model(x.to(device), return_hidden_states=True)
            viz.add(Trajectory(hidden_states=aux["hidden_states"]))
